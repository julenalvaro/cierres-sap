...
def transformar_bom_a_arbol_correcciones_EA(bom, coois, fabr_real, stocks):
...
def transformar_bom_a_arbol_correcciones_EB(bom, coois, fabr_real, stocks):
    # Verificar y renombrar columnas
    if "Texto breve-objeto" in bom.columns:
        bom.rename(columns={"Texto breve-objeto": "Texto breve"}, inplace=True)
    ...
    # RELLENAMOS A MANO LAS FASES EN EL ARCHIVO DE BOM-EB
    ...
    #     (bom["Versión fabricación"] != 0) &
    #     (~bom["Grupo de artículos"].isin([
    #         "Z00170000", "Z00440000", "Z00450000"
    #     ]))
    # ].copy() # asegurar que estamos trabajando con un DataFrame independiente

    # # Añadir columna "fase"
    # bom.loc[:, "fase"] = bom.apply(
    # lambda row: "B1" if "ARMAR+SOLDAR" in row["Nº componentes"] else
    #             "B2" if "M.SOPOR.+RECOCER" in row["Nº componentes"] else
    #             "B4" if "MEC. BASTIDOR" in row["Nº componentes"] else
    #             "PINTURA" if pd.notnull(row["Texto breve"]) and "chorr" in row["Texto breve"].lower() else
    #             "SUBCONJUNTOS" if row["Grupo de artículos"] == "Z00210000" else
    #             None, axis=1)
